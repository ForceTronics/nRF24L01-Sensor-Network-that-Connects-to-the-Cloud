...
/******************************************************************/
  ...
  RF24NetworkNoSleep::RF24NetworkNoSleep( RF24& _radio ): radio(_radio), frame_size(MAX_FRAME_SIZE)
  #else
  RF24NetworkNoSleep::RF24NetworkNoSleep( RF24& _radio, RF24& _radio1 ): radio(_radio), radio1(_radio1),frame_size(MAX_FRAME_SIZE)
  #endif
{
  txTime=0; networkFlags=0; returnSysMsgs=0; multicastRelay=0;
}
#elif !defined (DUAL_HEAD_RADIO)
RF24NetworkNoSleep::RF24NetworkNoSleep( RF24& _radio ): radio(_radio), next_frame(frame_queue)
{
...
#else
RF24NetworkNoSleep::RF24NetworkNoSleep( RF24& _radio, RF24& _radio1 ): radio(_radio), radio1(_radio1), next_frame(frame_queue)
{
  #if !defined ( DISABLE_FRAGMENTATION )
  frag_queue.message_buffer=&frag_queue_message_buffer[0];
  frag_ptr = &frag_queue;
...
void RF24NetworkNoSleep::begin(uint8_t _channel, uint16_t _node_address )
{
...
#if defined ENABLE_NETWORK_STATS
void RF24NetworkNoSleep::failures(uint32_t *_fails, uint32_t *_ok){
	*_fails = nFails;
	*_ok = nOK;
}
#endif
...
uint8_t RF24NetworkNoSleep::update(void)
{
...
uint8_t RF24NetworkNoSleep::enqueue(RF24NetworkHeader* header) {
...
bool RF24NetworkNoSleep::appendFragmentToFrame(RF24NetworkFrame frame) {
...
bool RF24NetworkNoSleep::available(void)
{
#if defined (RF24_LINUX)
...
uint16_t RF24NetworkNoSleep::parent() const
{
  if ( node_address == 0 )
    return -1;
  else
...
uint16_t RF24NetworkNoSleep::peek(RF24NetworkHeader& header)
{
  if ( available() )
  {
  #if defined (RF24_LINUX)
...
uint16_t RF24NetworkNoSleep::read(RF24NetworkHeader& header,void* message, uint16_t maxlen)
{
  uint16_t bufsize = 0;
...
#if defined RF24NetworkMulticast
/******************************************************************/
bool RF24NetworkNoSleep::multicast(RF24NetworkHeader& header,const void* message, uint16_t len, uint8_t level){
	// Fill out the header
  header.to_node = 0100;
  header.from_node = node_address;
...
/******************************************************************/
bool RF24NetworkNoSleep::write(RF24NetworkHeader& header,const void* message, uint16_t len){
	return write(header,message,len,070);
}
/******************************************************************/
bool RF24NetworkNoSleep::write(RF24NetworkHeader& header,const void* message, uint16_t len, uint16_t writeDirect){
...
bool RF24NetworkNoSleep::_write(RF24NetworkHeader& header,const void* message, uint16_t len, uint16_t writeDirect)
{
  // Fill out the header
  header.from_node = node_address;
...
bool RF24NetworkNoSleep::write(uint16_t to_node, uint8_t directTo)  // Direct To: 0 = First Payload, standard routing, 1=routed payload, 2=directRoute to host, 3=directRoute to Route
{
  bool ok = false;
...
bool RF24NetworkNoSleep::logicalToPhysicalAddress(logicalToPhysicalStruct *conversionInfo){
...
bool RF24NetworkNoSleep::write_to_pipe( uint16_t node, uint8_t pipe, bool multicast )
{
  bool ok = false;
...
bool RF24NetworkNoSleep::is_direct_child( uint16_t node )
{
  bool result = false;
...
bool RF24NetworkNoSleep::is_descendant( uint16_t node )
{
...
void RF24NetworkNoSleep::setup_address(void)
{
...
/******************************************************************/
uint16_t RF24NetworkNoSleep::addressOfPipe( uint16_t node, uint8_t pipeNo )
{
...
uint16_t RF24NetworkNoSleep::direct_child_route_to( uint16_t node )
{
...
bool RF24NetworkNoSleep::is_valid_address( uint16_t node )
{
  bool result = true;
...
/******************************************************************/
#if defined (RF24NetworkMulticast)
void RF24NetworkNoSleep::multicastLevel(uint8_t level){
